...
const MessageItem = ({
  ...
  const { isDragging, style, bind } = useSwipe({
    onSwip: () => {
      onReply(msg);
    ...
    <div className="flex flex-col w-full overflow-x-hidden">
      ...
      >
        ...
        <div className="flex flex-col max-w-[70%] gap-1">
          ...
          {msg.content && (
            <div
              className={`p-3.5 chat-shadow-sm rounded-2xl flex flex-col gap-1 ${isSentByCurrentUser
          ...
          {msg.attachments?.length > 0 && (
            <div className="flex flex-col gap-2 mt-1">
              {msg.attachments.map((file, j) => {
                const isAudio =
                  file.url.match(/\.(mp3|wav|m4a|ogg|webm)(\?.*)?$/i) ||
                  file.url.includes("/video/upload/");
                return (
                  <div
                    key={`${file._id}_${j}`}
                    className={`p-1 rounded-2xl overflow-hidden ${isAudio
                      ? ""
                      : isSentByCurrentUser
...
const ChatList = ({ messages, userId, onReply, inputRef }) => {
  ...
  return (
    <div
      className="w-full flex flex-col font-comfortaa"
      onClick={() => {
        if (highlightedMsgId) setHighlightedMsgId(null);
      }}
    >
      {messages?.map((msg, i) => {
        const senderId =
